...
class TestClosure(unittest.TestCase):
    ...
    def test_dynamic_closure(self):
        ...
        self.assertEqual(makeObject(run_code('a := 1; f := (c) @=> a = c + 1; f(1); a', False, False, False)), 2)
        self.assertEqual(makeObject(run_code('a := 1; f := fn@ (c = 1) a = a + 1; f(); a', False, False, False)), 2)
